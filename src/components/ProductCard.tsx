...
export default function ProductCard({ producto }: ProductCardProps) {
  ...
  return (
    <>
      {/* Card V2 — Glassmorphism Apple */}
      <div
        onClick={() => { setModalOpen(true); setActiveImg(0); }}
        className={`group cursor-pointer overflow-hidden ${
          isPremium ? "v2-card-premium" : "v2-card"
        }`}
      >
        {/* Product image */}
        <div className="relative aspect-square overflow-hidden rounded-t-[20px] bg-gradient-to-br from-slate-100 via-white to-blue-50">
          {producto.imagenes[0] ? (
            <Image
              ...
              className="object-cover transition-transform duration-[700ms] ease-out group-hover:scale-[1.10]"
              onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }}
            />
          ) : (
            <div className="absolute inset-0 flex items-center justify-center text-slate-300 text-xs font-medium">
              Sin foto
            </div>
          )}

          {/* Gradient overlay on hover */}
          <div className="absolute inset-0 bg-gradient-to-t from-slate-950/40 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-400 pointer-events-none" />

          {/* Store badge — floating glass on top-left */}
          <div className="absolute top-2.5 left-2.5 v2-glass-strong rounded-full pl-1.5 pr-2.5 py-1 flex items-center gap-1.5 max-w-[80%]">
            {local.logoUrl ? (
          ...
          {/* Premium star — floating top-right */}
          {isPremium && (
          ...
          <button
            ...
            aria-label="Me gusta"
            className={`absolute bottom-2.5 right-2.5 w-9 h-9 rounded-full v2-glass-strong flex items-center justify-center transition-all duration-300 hover:scale-110 ${
              liked ? "opacity-100 scale-100" : "opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0"
            }`}
          >
            <Heart className={`w-4 h-4 transition-colors ${liked ? "fill-rose-500 text-rose-500" : "text-slate-600"}`} />
          </button>
        </div>

        {/* Product info */}
        <div className="px-3.5 py-3">
          <h3 className="text-[13px] font-semibold text-slate-800 leading-snug line-clamp-2 min-h-[2.4rem]">
            {producto.nombre}
          </h3>
          <div className="mt-2 flex items-baseline gap-2 flex-wrap">
            ...
            {producto.precioAnterior && (
              <span className="ml-auto inline-flex items-center px-1.5 py-0.5 rounded-md bg-emerald-50 text-emerald-700 text-[10px] font-bold">
          ...
          <div className="mt-2 flex items-center gap-1 text-slate-400">
            ...
            <span className="text-[10.5px] font-medium">{local.ubicacion}</span>
          </div>
        </div>
      </div>
